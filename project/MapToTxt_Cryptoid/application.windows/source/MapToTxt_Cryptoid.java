...
public class MapToTxt_Cryptoid extends PApplet {
...
public void setup(){
  ...
  try{
    ...
    if(fileLeads == null || fileType == null || colorsFile == null)throw new Exception();
  }catch(Exception e){
    //if there is a problem opening one of the files
    addError("Error reading 'fileLeads.txt', 'fileType.txt', or 'colorMap.txt'");
    exit();
    return;
  }
...
public String ConvertMap(String name){
  //load the image
  PImage im;
  ...
  saveStrings(dataPath("../"+terrPath+name), dataLines);
...
public void addError(String e){
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "MapToTxt_Cryptoid" });
  }
}
